...
<li class="list-group-item " id="todo-list-{{$todolist->id}}">
    ...
    <h4 class="list-group-item-heading">{{ $todolist->title }} <span class="badge">{{ $task_count }} {{$task_count>1 ? 'tasks': 'task'}}</span></h4>
    <p class="list-group-item-text">{{$todolist->description}}</p>
    <div class="buttons">
        <a href="{{route('todolists.show', $todolist->id)}}" class="btn btn-info show-task-modal btn-xs" data-action=" {{ route('todolists.tasks.store', $todolist->id) }} " data-title="{{ $todolist->title }}" title="Manage Tasks">
